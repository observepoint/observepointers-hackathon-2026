...
export const SIDEBAR_ANCHORS = new Set([
  ANCHOR.navCreateNew,
  // Part 1's sidebar selectors, added at the integration merge. Without them the
  // nav-available guard silently did not apply to any Part 1 walkthrough -- which is
  // the failure mode the appliesTo comment below warns about, arrived at from the
  // other direction: the guard was fine, the anchor list just did not know about the
  // steps that needed it. Every Part 1 recipe starts on one of these.
  //
  // Kept alongside the guide-* ids rather than replacing them: those are what Part 2's
  // own recipes target, and #guide-left-nav-create-new never renders anyway (the
  // sidebar binds op-selector but not id), so removing it would fix nothing.
