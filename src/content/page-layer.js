  ...
  for (const plan of plans) {
    ...
    try {
      while (stepIndex < plan.steps.length) {
        ...
        // waiting on them, stop waiting on a step they can no longer see and re-run it once
        // the state is back, rather than sitting on a dead highlight.
        let interrupted = false

        if (step.actor === 'ai') {
          await executeAiAction(element, step.action, step.dwellMs)
          // No race for an 'ai' step -- it completes on its own -- but the dwell can be
          // seconds long, so consult the monitor once it is done.
          interrupted = Boolean(monitor.violatedFor(step))
        } else {
          const finished = isDomMutation ? nextPromise : waitForCompletion(step, signal, element)
          // Boolean(), not () => true: nextViolationFor resolves null on shutdown, which is
          // not an interruption to retry.
    ...
    if (missedRequired) {
      showPrerequisitePopup(plan.goal, missedRequired.say)
...
function waitForCompletion(step, signal, stepElement) {
  ...
  if (completion.type === 'click') {
    return new Promise(resolve => {
      ...
      // selector than the step highlights.
      const watchesStepTarget =
        !completion.targetSelector || completion.targetSelector === step.targetSelector

      const target = (watchesStepTarget && stepElement) || findVisible(rawSelector)
