 ...
 *
 * SELECTORS
 *   Solid — sidebar links (opLinkSelectorMap); cc-name, cc-create-next,
 *           cc-create-save (ConsentCategoriesUIConstants.selectors). cc-name is
 *           bound onto the <input matInput> itself, so it needs no descend.
 *   Weak  — the create menu. op-button-2021 renders menuItems into a mat-menu
 *           with no per-item op-selector, so we point at the open panel and let
 *           the text fallback name the item.
 */
export default {
